...
function createAnnotationCard(parent, cardData, handlers) {
...
var AnnotationSidebarView = class extends import_obsidian12.ItemView {
  ...
  async renderCards() {
    ...
    if (sorted.length === 0) {
      ...
      this.renderEmpty(
        this.cardListEl,
        this.searchQuery || this.colorFilter !== "all" ? loc.sidebarNoMatch : loc.sidebarNoAnnotations
      );
      return;
    }
  ...
  async loadAllAnnotations() {
    ...
    const annotationsDir = (0, import_obsidian12.normalizePath)(`${pluginDir}/annotations`);
    ...
    const worker = async () => {
      while (cursor < mdFiles.length) {
        ...
        try {
          ...
          const originalFile = this.app.vault.getAbstractFileByPath(notePath);
          if (!(originalFile instanceof import_obsidian12.TFile)) continue;
          const annotations = await this.fileManager.getAnnotations(notePath);
  ...
  applyFilters(cards) {
    ...
    if (this.searchQuery) {
      ...
      result = result.filter((c) => {
        ...
        const note = (_b = (_a = c.annotation.note) == null ? void 0 : _a.toLowerCase()) != null ? _b : "";
  ...
  renderDetailContent() {
    ...
    textCopyBtn.addEventListener("click", () => {
      void navigator.clipboard.writeText(annotation.text).then(() => {
        ...
        window.setTimeout(() => {
          textCopyBtn.textContent = loc.copy;
        }, 1500);
      });
    });
      ...
      if (annotation.note) {
        ...
        noteCopyBtn.addEventListener("click", () => {
          void navigator.clipboard.writeText(annotation.note).then(() => {
            ...
            window.setTimeout(() => {
              noteCopyBtn.textContent = loc.sidebarNoteCopyRestore;
            }, 1500);
          });
        });
      }
  ...
  async handleDetailSave() {
    ...
    if (view && view.getMode() === "source") {
      edited = await editAnnotationInEditor(view, this.fileManager, notePath, annotation.id, {
        ...
        note: this.editNote,
        rubyTexts: this.editRubyTexts.length > 0 ? this.editRubyTexts : void 0,
        isFullText: annotation.isFullText,
        isCrossBlock: annotation.isCrossBlock
      });
    }
    ...
    this.closeDetailPanel();
    new import_obsidian12.Notice(t().noticeAnnotationUpdated);
  }
  // ========== 查找标注视图 ==========
  findAnnotationView(notePath) {
    ...
    this.app.workspace.iterateAllLeaves((leaf) => {
      ...
      if (view instanceof import_obsidian12.MarkdownView && ((_a = view.file) == null ? void 0 : _a.path) === annotationPath) {
        result = view;
      }
    });
    return result;
  ...
  async handleCardOpen(cardData) {
    ...
    if (!targetLeaf) {
      ...
      if (!(file instanceof import_obsidian12.TFile)) {
        new import_obsidian12.Notice(t().noticeNoteFileNotFound);
        return;
      }
  ...
  handleCardDelete(cardData) {
    ...
    new ConfirmOverwriteModal(
      ...
      async () => {
        ...
        await this.plugin.refreshAnnotationView(notePath);
        this.closeDetailPanel();
        new import_obsidian12.Notice(loc.noticeDeleted);
      },
      loc.delete
    ).open();
  }
  ...
  async exportCurrentAnnotations() {
    ...
    if (cards.length === 0) {
      new import_obsidian12.Notice(loc.noData);
      return;
    }
    ...
    const doExport = (folderPath) => {
      ...
      new FileNameModal(this.app, noteName, async (fileName) => {
        const filePath = (0, import_obsidian12.normalizePath)(folderPath && folderPath !== "/" ? `${folderPath}/${fileName}` : fileName);
        ...
        const doWrite = async () => {
          try {
            ...
            new import_obsidian12.Notice(loc.noticeExportSuccess(annotations.length));
          } catch (e) {
            console.error("\u5BFC\u51FA\u5931\u8D25:", e);
            new import_obsidian12.Notice(loc.noticeExportFailed);
          }
        };
        if (existing instanceof import_obsidian12.TFile) {
          new ConfirmOverwriteModal(
...
async function preScanOldAnnotations(app, pluginDir) {
  const annotationsDir = (0, import_obsidian13.normalizePath)(`${pluginDir}/annotations`);
...
var ImportConfirmModal = class extends import_obsidian14.Modal {
...
var AnnotationPlugin = class extends import_obsidian15.Plugin {
  constructor() {
    super(...arguments);
    // 原始文件路径 → 标注文件路径的映射
    ...
    if (this.activeAnnotationSessions.size > 0) {
      ...
      for (const [originalPath, annotationPath] of this.activeAnnotationSessions) {
        ...
        this.app.workspace.iterateAllLeaves((leaf) => {
          var _a;
          const view = leaf.view;
          if (view instanceof import_obsidian15.MarkdownView && ((_a = view.file) == null ? void 0 : _a.path) === annotationPath) tabCount++;
        });
  ...
  async _doRecoverOrphanedTabs() {
    ...
    for (const key of Object.keys(savedSessions)) {
      ...
      if (!annotationExists || !(originalFile instanceof import_obsidian15.TFile)) continue;
  ...
  updateAnnotationTabTitle(leaf, originalPath) {
    var _a;
    const originalFile = this.app.vault.getAbstractFileByPath(originalPath);
    const displayName = originalFile instanceof import_obsidian15.TFile ? originalFile.basename : (_a = originalPath.replace(/\.md$/, "").split("/").pop()) != null ? _a : originalPath;
  ...
  async toggleAnnotationView() {
    ...
    const currentFile = view == null ? void 0 : view.file;
    if (!currentFile) {
      new import_obsidian15.Notice(t().noticeNoFile);
      return;
    }
  ...
  async openAnnotationView(leaf, notePath) {
    ...
    if (!ok) {
      new import_obsidian15.Notice(t().noticeFileCreateFailed);
      return;
    }
    const annotationPath = (0, import_obsidian15.normalizePath)(this.fileManager.getAnnotationFilePath(notePath));
    ...
    try {
      ...
      new import_obsidian15.Notice(t().noticeOpenFailed + (e instanceof Error ? e.message : String(e)));
    }
  }
  async closeAnnotationView(leaf, originalPath) {
    ...
    const originalFile = this.app.vault.getAbstractFileByPath(originalPath);
    if (!(originalFile instanceof import_obsidian15.TFile)) {
      new import_obsidian15.Notice(t().noticeOriginalMissing);
  ...
  collectSelectionParams(selection) {
    ...
    if (isCrossSection) {
      ...
      if (startCallout !== endCallout) {
        new import_obsidian15.Notice(t().noticeNoCrossCallout);
        return null;
      }
    }
  ...
  async annotateSelectionWithColor(color) {
    ...
    if (((_a = view == null ? void 0 : view.getMode) == null ? void 0 : _a.call(view)) === "source" && view.editor) {
      ...
      if (!text) {
        new import_obsidian15.Notice(loc.noticeNoSelection);
        return;
      }
      const from = view.editor.getCursor("from");
      const to = view.editor.getCursor("to");
      if (from.line !== to.line) {
        new import_obsidian15.Notice(loc.noticeMultiLineSelection);
        return;
      }
  ...
  setupAnnotationListPanel(notePath, leaf) {
    const view = leaf.view instanceof import_obsidian15.MarkdownView ? leaf.view : null;
    if (!view) return;
  ...
  async refreshAnnotationView(notePath) {
    ...
    try {
      ...
      this.app.workspace.iterateAllLeaves((leaf) => {
        var _a2;
        const v = leaf.view;
        if (v instanceof import_obsidian15.MarkdownView && ((_a2 = v.file) == null ? void 0 : _a2.path) === annotationPath) {
          leaves.push(leaf);
        }
      });
  ...
  registerImageEmbedFixer() {
    ...
    this.registerMarkdownPostProcessor((el, ctx) => {
      ...
      for (const embed of Array.from(embeds)) {
        ...
        if (!(target instanceof import_obsidian15.TFile)) continue;
      ...
      this.app.vault.on("rename", async (file, oldPath) => {
        if (file instanceof import_obsidian15.TFile && file.extension === "md") {
          ...
          if (annotationPath) {
            this.removeFakeTFile(annotationPath);
            this.removeMetadataCache(annotationPath);
            const newAnnotationPath = (0, import_obsidian15.normalizePath)(
  ...
  registerCommands() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (scan.fileCount === 0) {
          new import_obsidian15.Notice(t().noticeNoLegacyData);
          return;
        }
  ...
  async exportCurrentAnnotations(notePath) {
    ...
    const hasFile = await this.fileManager.hasAnnotationFile(notePath);
    if (!hasFile) {
      new import_obsidian15.Notice(loc.noData);
      return;
    }
    const annotations = await this.fileManager.getAnnotations(notePath);
    if (annotations.length === 0) {
    ...
    const doExport = (folderPath) => {
      const originalFile = this.app.vault.getAbstractFileByPath(notePath);
      const noteName = originalFile instanceof import_obsidian15.TFile ? originalFile.name.replace(/\.md$/, "") : "";
      new FileNameModal(this.app, noteName, async (fileName) => {
        const filePath = (0, import_obsidian15.normalizePath)(folderPath && folderPath !== "/" ? `${folderPath}/${fileName}` : fileName);
        const existing = this.app.vault.getAbstractFileByPath(filePath);
        const doWrite = async () => {
          ...
          new import_obsidian15.Notice(loc.noticeExportSuccess(sorted.length));
        };
        if (existing instanceof import_obsidian15.TFile) {
          new ConfirmOverwriteModal(
            this.app,
