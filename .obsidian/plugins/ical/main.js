...
var Logger = class _Logger {
  ...
  log(message, object) {
    if (this.isDebug) {
      console.debug("[" + (0, import_obsidian.moment)().format("YYYY-MM-DD-HH:mm:ss.SSS") + "][info][ical] " + message);
      if (object) {
...
var SecretStore = class _SecretStore {
  ...
  static defaultStorage() {
    const ls = typeof window === "undefined" ? void 0 : window.localStorage;
    if (ls) return ls;
...
var IcalService = class {
  ...
  getEvent(task, date, prependSummary, taskDateName = null) {
    if (task.hasAnyDate() === false) {
      return "";
    }
    let event = "BEGIN:VEVENT\r\nUID:" + task.getId(taskDateName != null ? taskDateName : "") + "\r\nDTSTAMP:" + task.getDate(null, "YYYYMMDDTHHmmss") + "\r\n";
    if (date === null) {
      switch (settings.howToProcessMultipleDates) {
        ...
        case "CreateMultipleEvents":
          ...
          if (task.hasA("Start" /* Start */)) {
            event += this.getEvent(task, task.getDate("Start" /* Start */, "YYYYMMDD"), "\u{1F6EB} ", "Start" /* Start */);
          }
          if (task.hasA("Scheduled" /* Scheduled */)) {
            event += this.getEvent(task, task.getDate("Scheduled" /* Scheduled */, "YYYYMMDD"), "\u23F3 ", "Scheduled" /* Scheduled */);
          }
          if (task.hasA("Due" /* Due */)) {
            event += this.getEvent(task, task.getDate("Due" /* Due */, "YYYYMMDD"), "\u{1F4C5} ", "Due" /* Due */);
          }
          if (task.hasA("Done" /* Done */)) {
            event += this.getEvent(task, task.getDate("Done" /* Done */, "YYYYMMDD"), "\u2705 ", "Done" /* Done */);
          }
          if (event === "") {
...
var Task = class {
  ...
  getId(discriminator = "") {
    const dateKey = this.dates.map((taskDate) => `${taskDate.name}:${taskDate.date.getTime()}`).sort().join(",");
    const input = discriminator === "" ? `${this.fileUri}::${this.summary}::${dateKey}` : `${this.fileUri}::${this.summary}::${dateKey}::${discriminator}`;
    let h1 = 2166136261 >>> 0;
    let h2 = 2654435769 >>> 0;
