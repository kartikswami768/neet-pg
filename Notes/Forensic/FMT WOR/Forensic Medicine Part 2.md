...
```mermaid
flowchart TD
    A[Asphyxial deaths] --> B[Hanging]
    A --> C[Strangulation]
    A --> D[Suffocation]
    B --> B1[Neck compression due to body-weight suspension]
    C --> C1[Only neck compression; no body suspension]
    D --> D1[No neck compression]
    C --> C2[Ligature strangulation]
    C --> C3[Manual strangulation / throttling]
    C --> C4[Mugging: elbow / forearm]
    C --> C5[Bansdola: bamboo stick]
    C --> C6[Garrotting: thin ligature cord]
    C6 --> C7[Spanish windlass technique]
```
...
```mermaid
flowchart TD
    A[Hyoid bone fracture] --> B[Adduction fracture - most common]
    A --> C[Abduction / AP compression fracture]
    A --> D[Side-to-side compression fracture]
    B --> B1[Inward compression]
    B --> B2[Seen in throttling]
    C --> C1[Outward compression]
    C --> C2[Seen in hanging]
    D --> D1[One end displaced outwards and the other inwards]
    D --> D2[Seen in hanging]
```
...
```mermaid
flowchart TD
    A[Order of autopsy in hanging] --> B[Cranium opened first]
    B --> B1[To avoid Prinsloo-Gordon artefact]
    A --> C[Neck opened last]
    C --> C1[To facilitate bloodless dissection of neck]
```
...
```mermaid
flowchart TD
    A[Manner of hanging] --> B[Suicidal - most common]
    A --> C[Homicidal]
    A --> D[Accidental]
    B --> B1[Most common method of suicide: hanging > poisoning]
    C --> C1[Lynching / extrajudicial hanging]
    C1 --> C2[Done by a mob]
    D --> D1[Sexual asphyxia]
    D1 --> D2[Asphyxiophilia / hypoxiphillia / Kotzwarism]
...
```mermaid
flowchart TD
    A[Drowning] --> B[Dry drowning]
    A --> C[Wet / typical drowning]
    A --> D[Hydrocution / immersion syndrome]
    A --> E[Secondary drowning / post-immersion syndrome]
    B --> B1[Reflex vocal cord spasm]
    B1 --> B2[Blocks air entry]
    B2 --> B3[Asphyxia]
    C --> C1[Water enters lung]
    C1 --> C2[Pulmonary surfactant washed away]
    C2 --> C3[Alveolar collapse]
    C3 --> C4[Cerebral anoxia]
    D --> D1[Person falls into cold water]
    D1 --> D2[Stimulation of vagus nerve]
    D2 --> D3[Bradycardia]
    D3 --> D4[Cardiac arrest]
    E --> E1[Person rescued from drowning]
    E1 --> E2[Later dies due to complications]
    E2 --> E3[Infections / hypoxic ischemic encephalopathy / aspiration]
```
...
```mermaid
flowchart TD
    A[Duration] --> B[<20 weeks]
    A --> C[20-24 weeks]
    A --> D[>24 weeks]
    B --> B1[1 doctor approved]
    B --> B2[All indications]
    C --> C1[2 doctors approved]
    C --> C2[Rape/incest]
    C --> C3[Minor]
    C --> C4[Marital status change]
    C --> C5[Mental/physical disability]
    C --> C6[Mass disaster]
    D --> D1[Medical board approval]
    D1 --> D2[Obstetrician]
    D1 --> D3[Paediatrician]
    D1 --> D4[Radiologist]
    D1 --> D5[Independent member appointed by state government]
    D --> D6[Only in substantial fetal anomaly]
```
...
```mermaid
flowchart TD
    A[Test for blood stains] --> B[Presumptive tests]
    A --> C[Confirmatory tests]
    A --> D[Luminescent test]
    B --> B1[Phenolphthalein / Kastle-Meyer]
    B1 --> B2[Pink color +]
    B --> B3[Benzidine / orthotoluidine]
    B3 --> B4[Blue color +]
    C --> C1[Microscopy]
    C --> C2[Microchemical tests]
    C2 --> C21[Teichmann test: brown rhombic haemin crystals]
    C2 --> C22[Takayama test: pink feathery haemochromogen crystals]
    C --> C3[Absorption spectrometry - most reliable]
    D --> D1[Luminol spray]
    D --> D2[UV light]
```
...
```mermaid
flowchart TD
    A[Corrosives] --> B[Acids]
    A --> C[Alkali]
    B --> B1[Coagulative necrosis]
    B --> B2[Except hydrofluoric acid: liquefactive necrosis]
    B --> B3[Inorganic: no systemic absorption, only local effects]
    B --> B4[Organic: systemic + local effects]
    B3 --> B31[Sulphuric acid]
    B3 --> B32[Nitric acid]
    B3 --> B33[Hydrochloric acid]
    B4 --> B41[Carbolic acid]
    B4 --> B42[Oxalic acid]
    B4 --> B43[Acetic acid]
    C --> C1[Liquefactive necrosis]
    C --> C2[More dangerous due to increased lateral spread]
    C2 --> C3[Sodium hydroxide]
    C2 --> C4[Potassium hydroxide]
    C2 --> C5[Potassium carbonate]
    C2 --> C6[Sodium carbonate]
```
...
```mermaid
flowchart LR
    A[Methanol] --> B[Alcohol dehydrogenase]
    B --> C[Formaldehyde]
    C --> D[Aldehyde dehydrogenase]
    D --> E[Formic acid]
    E --> F[Retina / optic nerve involvement]
    E --> G[HAGMA]
    A --> H[↑Lactic acid production]
```
...
```mermaid
flowchart TD
    A[CO] --> B[Carboxyhemoglobin]
    B --> C[O2 dissociation curve shifts left]
    C --> D[Less O2 reaches cells]
    D --> E[Anaemic anoxia]
    E --> F[Anaerobic metabolism]
    F --> G[↑ lactic acid]
    G --> H[Acidosis]
    A --> I[Cytochrome oxidase]
    A --> J[Myoglobin]
    J --> K[Rebound toxicity]
```
...
```mermaid
flowchart LR
    A[Venomous snakes] --> B[Elapidae]
    A --> C[Viperidae]
    A --> D[Hydrophidae]
    B --> B1[King cobra]
    B --> B2[Common cobra]
    B --> B3[Krait]
    C --> C1[Russell's viper]
    C --> C2[Saw-scaled viper]
    D --> D1[Sea snake]
```
...
```mermaid
flowchart TD
    A[Snake bite first aid - RIGHT] --> B[Reassure patient]
    A --> C[Immobilize entire limb]
    C --> C1[Crepe bandage / cloth]
    C1 --> C2[Occlude lymphatics: 55-70 mmHg]
    A --> D[Get to hospital immediately]
    A --> E[Tell doctor about systemic symptoms on the way]
```
...
```mermaid
flowchart TD
    A[Organ donation] --> B[Before death]
    A --> C[After death]
    B --> B1[Any donor >18 years]
    B --> B2[Authorized donor]
    B --> B3[Legal guardian]
    C --> C1[After brainstem death]
    C --> C2[Non-authorized donor]
    C --> C3[Legal guardian]
    C --> C4[Unclaimed bodies >48 h]
    C4 --> C5[Hospital management]
    C --> C6[In MLC]
    C6 --> C7[Autopsy doctor]
```
